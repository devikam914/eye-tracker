...
def force_foreground(window_title):
    ...
    try:
        ...
        pass   # non-Windows or window not found yet ΓÇö silently ignore
...
SNAP_RADIUS = 40   # px ΓÇö how far to search for a nearby element
...
def snap_to_nearest_element(x, y):
    ...
    Filters out huge windows (whole app panels) ΓÇö keeps only small controls
    like buttons, title-bar icons, toolbar items.
    Returns the center of the nearest qualifying control, or (x, y) unchanged.

    Works without any extra pip installs ΓÇö pure Win32 via ctypes.
...
DWELL_RADIUS       = 45     # pixels   ΓÇö how much gaze can wander and still count
DWELL_COOLDOWN     = 1.2    # seconds  ΓÇö min gap between consecutive clicks
...
# Progress (0.0ΓÇô1.0) exposed for optional visual feedback.
class DwellClicker:
    def __init__(self):
        self.anchor_x    = None
        self.anchor_y    = None
        ...
        self.progress    = 0.0       # 0.0 ΓåÆ 1.0 for UI feedback
    ...
    def update(self, gx, gy):
        ...
        # Still in post-click cooldown ΓÇö don't accumulate dwell
        ...
        if dist > DWELL_RADIUS:
            # Gaze moved ΓÇö start fresh anchor at new position
            self._reset(gx, gy)
            return False

        # Gaze still inside radius ΓÇö accumulate dwell time
...
while True:
    ...
    if gx is not None:
        ...
        if ear < BLINK_THRESHOLD:
            ...
            # Dwell click ΓÇö runs on smoothed cursor position
