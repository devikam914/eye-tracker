...
class GazeFilter:
    def __init__(self):
        self.fx = OneEuroFilter(min_cutoff=1.0, beta=0.4)
        self.fy = OneEuroFilter(min_cutoff=1.0, beta=0.4)
...
for pt_idx, (px,py) in enumerate(calibration_points):
    ...
    raw = collect_samples(60, str(pt_idx+1)+"/"+str(N_PTS), tx, ty)
    s   = np.array(raw)
...
if bad_pts:
    ...
    for i in bad_pts:
        ...
        tx, ty = int(px*screen_w), int(py*screen_h)
        raw = collect_samples(60, "Recollect "+str(i+1), tx, ty)
        if raw:
...
while True:
    ...
    if gx is not None:
        ...
        if ear < BLINK_THRESHOLD:
            blink_cooldown = 20; gaze_filter.reset()
